...
bool V4L2Util::InitBuffer(int fd, V4L2BufferGroup *gbuffer, v4l2_buf_type type, v4l2_memory memory, bool has_dmafd) {
  v4l2_capability cap = {};
...
bool V4L2Util::SetFormat(int fd, V4L2BufferGroup *gbuffer, int width, int height, uint32_t &pixel_format) {
  v4l2_format fmt = {};
  ...
  if (ioctl(fd, VIDIOC_S_FMT, &fmt) < 0) {
    ERROR_PRINT("fd(%d) set format(%s) : %s", fd, V4L2Util::FourccToString(fmt.fmt.pix_mp.pixelformat).c_str(),
                strerror(errno));
    return false;
  }
...
bool V4L2Util::MMap(int fd, V4L2BufferGroup *gbuffer) {
  for (int i = 0; i < gbuffer->num_buffers; i++) {
    ...
    if (gbuffer->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE || gbuffer->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
      buffer->length = inner->m.planes[0].length;
      buffer->start =
              mmap(NULL, buffer->length, PROT_READ | PROT_WRITE, MAP_SHARED, fd, inner->m.planes[0].m.mem_offset);
    } else if (gbuffer->type == V4L2_BUF_TYPE_VIDEO_CAPTURE || gbuffer->type == V4L2_BUF_TYPE_VIDEO_OUTPUT) {
    ...
    DEBUG_PRINT("fd(%d) query buffer at %p (length: %d)", gbuffer->fd, buffer->start, buffer->length);
  }
