...
//     timestamp_(buffer.timestamp), buffer_(buffer), is_buffer_copied(false),
...
//     width_(width), height_(height), format_(format), size_(size), flags_(0), timestamp_({0, 0}),
//     is_buffer_copied(false), data_(static_cast<uint8_t *>(webrtc::AlignedMalloc(size_, kBufferAlignment))) {}
